...
class OrderSerializer(serializers.ModelSerializer):
    ...
    estimated_delivery_date = serializers.DateTimeField(read_only=True)  # Changed to DateTimeField for precision
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'order_id', 'user', 'session_key', 'coupon', 'status', 'total_price', 'items', 'estimated_delivery_date', 'created_at', 'updated_at']
    ...
    def validate(self, attrs):
        ...
        if not cart or not cart.items.exists():
            logger.warning(f"Checkout attempted with empty cart: user={request.user}, session_key={request.session.session_key}")
    ...
    def create(self, validated_data):
        ...
        # Set estimated_delivery_date to 12 hours from now
        estimated_delivery_date = timezone.now() + timedelta(hours=12)
        ...
        receipt_lines.extend([
            ...
            f"Estimated Delivery: {order.estimated_delivery_date.strftime('%Y-%m-%d %H:%M:%S')}",  # Show precise time
